...
export function parseSheetText(raw) {
  ...
  const data = {
    lastName: '', firstName: '', gender: '', age: '', house: '', origin: '',
    stats: {}, skills: { explore: [], social: [], action: [] },
    favSubject: '', weakSubject: '', appearance: '', personality: '',
    wand: '', pet: '', belongings: '', treasure: '', intro: ''
  }
  const simpleKeyMap = {
    'ファーストネーム': 'firstName', 'ファミリーネーム': 'lastName', '性別': 'gender', '年齢': 'age', '寮': 'house', '出身地': 'origin',
  ...
  data.name = [data.firstName, data.lastName].filter(Boolean).join('・') || data.firstName || data.lastName
  return data
}

export const SAMPLE_TEXT = `ファーストネーム：ミラ
ファミリーネーム：藤堂
性別：女性
年齢：19
寮：ハッフルパフ
