...
export function parseSheetText(raw) {
  ...
  const simpleKeyMap = {
    '姓': 'lastName', '名': 'firstName', '性別': 'gender', '年齢': 'age', '寮': 'house', '出身地': 'origin',
  ...
  data.name = (data.lastName + data.firstName).trim() || data.firstName || data.lastName
  return data
}

export const SAMPLE_TEXT = `姓：藤堂
名：ミラ
性別：女性
年齢：19
寮：ハッフルパフ
